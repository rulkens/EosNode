...
function Client(settings){
    ...
    this.socket.on('message', function (msg, remote) {
        ...
        // init message
        if(message.init){
            console.log('[UDP] message.init', message);
            ...
            console.log('[UDP] message.error', message);
...
function sendMessage (message){
    ...
    if(!this.connected && !this.connecting){
        console.log('[UDP] trying to connect');
...
function connect(cb) {
    if(this.connecting){
        console.log('[UDP] already connecting!');
        return;
    }
    ...
    this.socket.send(initMessage, 0, initMessage.length, this.port, this.host, function(err, bytes) {
        console.log('[UDP] CONNECT INFO SENT TO: ' + this.host + ':' + this.port);
...
function parseMessage(msg){
    ...
    if(msg.toString('ascii') === INIT_OK){
        // OK message!
        console.log('[UDP] SERVER INIT OK');
        ret.init = true;
        ret.type = INIT_OK;
    }
