...
colorLightObservable.offLoop = off;
...
function waveToColorLight (options) {
    ...
    return function (wave) {
        ...
        var settings = {
            color     : c,
            intensity : wave.y * (o.intensity || 1),
            position  : (o.position.base || 0.5)// + posNoise
        };

        return ColorLight(settings);
    }
}
 ...
 */
function off () {
    var l = ColorLight({color : 0, intensity : 0});
    return Rx.Observable.just(l);
}

function offLoop (options) {
    var o = options || {};

    var l = ColorLight({color : 0, intensity : 0});
    return rxUtil.loop(o).map(() => l);
}