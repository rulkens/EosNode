...
function Pong(options) {
    if (!(this instanceof Pong)) return new Pong(options);
    ...
    this.lights = [
        Light(_.extend({ position: 1.0, direction: -1 }, lightSettings)),
        Light(_.extend({ position: 0.0, direction:  1 }, lightSettings))
    ];
}
